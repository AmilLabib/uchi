...
import { Plus, X, BookOpen, Sun, Cloud, CloudRain, Snowflake, Wind, Clock, ShoppingBag, Camera } from 'lucide-react'
import { useJournalStore, usePerfumeStore, useTimelineStore } from '../store/useStore'
import type { JournalEntry, TimelineEvent } from '../types'
  ...
  return (
    <div className="min-h-screen pt-28 md:pt-32 pb-20 px-6">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="page-header text-center mb-12 md:pt-16">
          <span className="text-xs font-medium tracking-[0.3em] uppercase text-accent">Personal Journey</span>
          <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl text-text-primary font-medium mt-4">
            My <span className="italic text-gradient">Story</span>
          </h1>
          <p className="mt-4 text-text-secondary max-w-md mx-auto text-sm md:text-base">
            Perjalanan aromamu dalam jurnal harian dan timeline. Setiap catatan adalah cerita yang tak terlupakan.
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          <button
            ...
            className={`px-6 py-3 text-sm font-medium tracking-wider uppercase transition-all duration-300 flex items-center gap-2 ${
              viewMode === 'journal' ? 'bg-accent text-primary' : 'bg-surface-card border border-border text-text-secondary hover:border-accent/30'
            }`}
          >
          ...
          <button
            ...
            className={`px-6 py-3 text-sm font-medium tracking-wider uppercase transition-all duration-300 flex items-center gap-2 ${
              viewMode === 'timeline' ? 'bg-accent text-primary' : 'bg-surface-card border border-border text-text-secondary hover:border-accent/30'
            }`}
          >
        ...
        <div ref={listRef} className="space-y-4">
          {viewMode === 'journal' ? (
            ...
            <TimelineView timeline={timeline} perfumes={perfumes} />
          )}
        </div>
      </div>
...
function TimelineView({ timeline, perfumes }: { timeline: TimelineEvent[]; perfumes: { id: string; name: string }[] }) {
    ...
    return (
      <div className="text-center py-28">
        ...
        <p className="text-sm text-text-muted mt-2">
          Timeline otomatis terisi dari koleksi parfum, momen, dan jurnal yang kamu buat
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="relative">
      {/* Center line */}
      <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-accent/50 via-accent/20 to-transparent md:-translate-x-px" />
